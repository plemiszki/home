...
if os.environ.get('FLASK_ENV') == 'production' and os.environ.get('TEMP_SENSOR_ENABLED') == 'true':
...
def api_albums(category):
    categories = ['modern', 'classical', 'christmas', 'jazz', 'soundtrack']
...
def start_music():
    ...
    if os.getenv('FLASK_ENV') == 'production':
        process_id = Popen(['mpv', '--audio-device=alsa/default', f"{music_directory}/{album.artist_name}/{album.name}/{filenames[track - 1]}"]).pid
...
def api_album_details(album_id):
    ...
    album_dict['category_name'] = ['modern', 'classical', 'christmas', 'jazz', 'soundtrack'][album.category - 1]
